                   ...
                   // var_dump($timeDiff);
                    if((($sublicense != null && $sublicense->getSubLicenseAppRole()->getRole() == AppRole::COMPANY_MANAGER) ||
                        ...
                        if($user_check != null){
                          ...
                          //  return new JsonResponse(array("status" => "error", "username already exists"));
                        ...
                        $user_new->setPlainPassword("test123");
                        ...
                        $token = $this->get('lexik_jwt_authentication.encoder.default')
                            ->encode(['username' => $user_new->getUsername()]);
    ...
    public function tokenAuthentication(Request $request)
    {
        if ($request->request) {
            ...
            if (!empty($content))
            {
                ...
                return new JsonResponse(array(["status" => "granted","token" => $token]));
            }
        }
        throw $this->createAccessDeniedException();
    }
